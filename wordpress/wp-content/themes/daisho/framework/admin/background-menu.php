			...
			if($value_main['group'] == $this_group){
				foreach($value_main['options'] as $key => $value){
					...
					if($value['type'] == 'colorpicker'){
						$color = '';
						if($field_values[$key_main][$value['css_property']]){ $color = 'style="background-color:'.$field_values[$key_main][$value['css_property']].';"'; }
						$field = '
							<tr>
								...
								<td>
									<input type="text" class="attcolorpicker flow_styling_input" name="flow_styling_'.$i.'" placeholder="'.$value['placeholder'].'" value="'.$field_values[$key_main][$value['css_property']].'">
						...
						$field = '
							<tr>
								<th><span class="flow-styling-title">'.$value['title'].'</span><span class="flow-styling-subtitle">(<b>'.$value['css_property'].'</b> for: '.$key_main.')</span></th>
								<td>
									<input class="flow_styling_input" name="flow_styling_'.$i.'" type="text" placeholder="'.$value['placeholder'].'" value="'.$field_values[$key_main][$value['css_property']].'">
								...
								<tr>
									...
									<td>
										<select class="flow_styling_input" name="flow_styling_'.$i.'">';
											if(empty($field_values[$key_main][$value['css_property']])){
											...
											foreach($value['options'] as $key_inner => $value_inner){
												if($key_inner == $field_values[$key_main][$value['css_property']]){
						...
						if(is_array($value['options'])){
							...
							if($field_values[$key_main][$value['css_property']]){
...
function flow_styling_field_names($styling_options, $this_group, $settings){
	if(is_array($styling_options) && isset($this_group) && is_array($settings)){
		$i = 0;
		foreach($styling_options as $key_main => $value_main){
			if($value_main['group'] == $this_group && $value['options']['type'] != 'group-label'){
				foreach($value_main['options'] as $key => $value){
...
function add_bg_menu(){
	...
	if($_POST['flow_styling_nonce_security'] && $_POST['flow_styling_nonce_security'] == "Y"){
