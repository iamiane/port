...
function flow_sidebar_filter($default_sidebar){
	...
	if(is_home()){
		...
		if(!empty($front_page)){
			//$post_ID_to_use = intval($front_page);
		}
	}
...
function flow_sidebar_about_meta_box(){ ?>
	...
	<p>If you wish to use this sidebar directly on page or in post, please use this shortcode: <code>[flow-sidebar id="flow-sidebar-ID"]</code></p>
	<?php
}
