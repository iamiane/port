        ...
        <?php }else{ /* comments are for registered and not registered users */ ?>
			...
			<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
				...
				<div class="respond-left-column">
					<label for="author">
						<?php _ex('Name', 'Comment form name field label', 'flowthemes'); ?><?php if($req){ _ex(' *', 'Required field symbol in comment form', 'flowthemes'); } ?>
					</label>
					<input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" class="input" size="50" placeholder="<?php _ex('Name', 'Comment form name field label', 'flowthemes'); ?><?php if($req){ _ex(' *', 'Required field symbol in comment form', 'flowthemes'); } ?>" />
					<label for="email">
						<?php _ex('Email', 'Comment form email field label', 'flowthemes'); ?><?php if($req){ _ex(' *', 'Required field symbol in comment form', 'flowthemes'); } ?>
					</label>
					<input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="50" class="input" placeholder="<?php _ex('Email', 'Comment form email field label', 'flowthemes'); ?><?php if($req){ _ex(' *', 'Required field symbol in comment form', 'flowthemes'); } ?>" />
					<label for="url">
						<?php _ex('Website', 'Comment form website field label', 'flowthemes'); ?>
					</label>
				...
				<div class="respond-right-column <?php if($user_ID){ ?>respond-right-column-full<?php } ?>">
					<p class="clearfix">
						<label for="comment"><?php _ex('Comment', 'Comment form textarea label', 'flowthemes'); ?></label>
						<textarea name="comment" id="data" cols="10" rows="6" tabindex="4" placeholder="<?php _ex('Comment', 'Comment form textarea label', 'flowthemes'); ?>"></textarea>
					</p>
					<p class="clearfix">
